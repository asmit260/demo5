...
(function () {
  ...
  async function loadBlog() {
    ...
    // Fetch the manifest — a list of blog post slugs generated by build or static index
    // Fallback: try loading the two sample posts directly
    const slugs = await safeFetch('/_data/blog/index.json');
    let posts = [];

    if (slugs && Array.isArray(slugs)) {
      // We have an index. Fetch each post.
      const fetches = slugs.map(s => safeFetch(`/_data/blog/${s}.md`, true).then(md => md ? parseFrontmatter(md, s) : null));
      const results = await Promise.all(fetches);
      posts = results.filter(Boolean);
    } else {
      // Try the two sample posts we know about
      const knownSlugs = [
